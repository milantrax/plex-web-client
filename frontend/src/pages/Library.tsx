...
import type { LibrarySyncStatus, PlexAlbum, PlexFilterDirectory } from '../types';

type FilterKind = 'genre' | 'year' | 'label';
...
function Library() {
  ...
  const [musicSectionId, setMusicSectionId] = useState<string | null>(null);
  ...
  const syncPollRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  const musicSectionIdRef = useRef<string | null>(null);

  const [genres, setGenres] = useState<PlexFilterDirectory[]>([]);
  const [years, setYears] = useState<PlexFilterDirectory[]>([]);
  const [labels, setLabels] = useState<PlexFilterDirectory[]>([]);
  ...
  const prevSyncStatusRef = useRef<LibrarySyncStatus | null>(null);
  ...
  const handleManualSync = async () => {
    try {
      ...
      setSyncStatus(s => ({ ...(s ?? { totalAlbums: 0, lastSyncedAt: null }), status: 'syncing', syncedAlbums: 0 }));
  ...
  const handleGenreChange = async (genreSelection: string) => {
  ...
  const handleYearChange = async (yearSelection: string) => {
  ...
  const handleLabelChange = async (labelSelection: string) => {
  ...
  const applyFilters = async (genreFilter: string, yearFilter: string, labelFilter: string) => {
    if (!musicSectionId) return;

    setLoading(true);
    try {
      let filteredAlbums: PlexAlbum[] = [];
