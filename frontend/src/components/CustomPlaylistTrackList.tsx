...
import type { CustomPlaylistPlexTrack, PlexTrack, TrackReorderEntry } from '../types';

interface SortableTrackRowProps {
  track: CustomPlaylistPlexTrack;
  index: number;
  isCurrent: boolean;
  isPlaying?: boolean;
  inQueue: boolean;
  onTrackClick: (track: CustomPlaylistPlexTrack) => void;
  onRemove: (track: CustomPlaylistPlexTrack) => void;
  onDownload: (e: React.MouseEvent<HTMLButtonElement>, track: CustomPlaylistPlexTrack) => void;
  onAddToQueue: (e: React.MouseEvent<HTMLButtonElement>, track: CustomPlaylistPlexTrack) => void;
  onOpenAddToPlaylist: (e: React.MouseEvent<HTMLButtonElement>, track: CustomPlaylistPlexTrack) => void;
  isDragOverlay?: boolean;
}

export interface CustomPlaylistTrackListProps {
  playlistId: number | string;
  tracks?: CustomPlaylistPlexTrack[] | null;
  onTracksChange: (tracks: CustomPlaylistPlexTrack[]) => void;
  onPlayTrack?: (track: CustomPlaylistPlexTrack) => void;
  /** Only the ratingKey is read, so any playing track is accepted. */
  currentTrack?: PlexTrack | null;
  isPlaying?: boolean;
  onTogglePlayback?: () => void;
  onRemoveTrack: (track: CustomPlaylistPlexTrack) => void;
  onTrackAddedToPlaylist?: (playlistId: number) => void;
}
    ...
    >
      ...
      <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center' }}>
        {(track.userRating || track.rating) ? (
          <Rating
            value={(track.userRating || track.rating || 0) / 2}
            precision={0.5}
            size="small"
            readOnly
...
export default function CustomPlaylistTrackList({
...
}: CustomPlaylistTrackListProps) {
  const [activeId, setActiveId] = useState<number | string | null>(null);
  const [saving, setSaving] = useState(false);
  const [queuedTracks, setQueuedTracks] = useState<Set<string>>(new Set());
  const [addToPlaylistTrack, setAddToPlaylistTrack] = useState<CustomPlaylistPlexTrack | null>(null);
  ...
  const isCurrent = (track: CustomPlaylistPlexTrack) =>
    !!currentTrack && String(currentTrack.ratingKey) === String(track.ratingKey);

  const isInQueue = (track: CustomPlaylistPlexTrack) => queuedTracks.has(track.ratingKey);

  const handleTrackClick = (track: CustomPlaylistPlexTrack) => {
    if (isCurrent(track)) {
  ...
  const handleDownload = async (e: React.MouseEvent<HTMLButtonElement>, track: CustomPlaylistPlexTrack) => {
    try {
  ...
  const handleAddToQueue = async (e: React.MouseEvent<HTMLButtonElement>, track: CustomPlaylistPlexTrack) => {
    try {
  ...
  const handleOpenAddToPlaylist = (e: React.MouseEvent<HTMLButtonElement>, track: CustomPlaylistPlexTrack) => {
  ...
  const handleDragEnd = async ({ active, over }: DragEndEvent) => {
    ...
    setSaving(true);
    try {
      const order: TrackReorderEntry[] = reordered.map((t, i) => ({ id: t._customTrackId, position: i }));
