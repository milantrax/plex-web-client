...
/** Callbacks invoked as the worker installs or updates. */
export interface ServiceWorkerConfig {
  onSuccess?: (registration: ServiceWorkerRegistration) => void;
  onUpdate?: (registration: ServiceWorkerRegistration) => void;
}

/** Per-cache size breakdown returned by `getCacheStorageEstimate`. */
...
export interface CacheStorageEstimate {
  ...
  caches: Record<string, CacheSizeInfo>;
}
 ...
 */
export async function getCacheStorageEstimate(): Promise<CacheStorageEstimate | null> {
  if ('caches' in window) {
    try {
      const cacheNames = await caches.keys();
      let totalSize = 0;
      const cacheSizes: Record<string, CacheSizeInfo> = {};
