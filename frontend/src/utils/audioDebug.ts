...
declare global {
  interface Window {
    debugPlexAudio?: (audioElement: HTMLAudioElement | null) => void;
  }
}

export const testPlexConnection = async (): Promise<boolean> => {
  try {
...
export const logAudioElementInfo = (audioElement: HTMLAudioElement | null): void => {
  if (!audioElement) {
...
export const initAudioDiagnostics = (): void => {
  console.log('Audio diagnostics initialized');

  window.debugPlexAudio = (audioElement: HTMLAudioElement | null) => {
