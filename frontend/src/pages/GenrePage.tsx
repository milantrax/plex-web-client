...
import type { PlexAlbum, PlexFilterDirectory } from '../types';

/** Router state carried in from the Genres list page. */
interface GenreLocationState {
  genreTitle?: string;
  sectionId?: string;
}
...
const groupAlbumsByYear = (albums: PlexAlbum[]): YearGroup[] => {
  const grouped = albums.reduce<Record<string, PlexAlbum[]>>((acc, album) => {
...
const GenrePage = () => {
  ...
  const locationState = location.state as GenreLocationState | null;

  const [genreTitle, setGenreTitle] = useState(locationState?.genreTitle || genreKey);
  const [genres, setGenres] = useState<PlexFilterDirectory[]>([]);
  const [albums, setAlbums] = useState<PlexAlbum[]>([]);
  const [sectionId, setSectionId] = useState<string | null>(locationState?.sectionId || null);
  ...
  useEffect(() => {
    const init = async () => {
      let sid = locationState?.sectionId;
      if (!sid) {
        try {
  ...
  useEffect(() => {
    ...
    const fetchAlbums = async () => {
      ...
      setSelectedYear('');
      try {
        if (locationState?.genreTitle) setGenreTitle(locationState.genreTitle);
  ...
  const handleGenreClick = (genre: PlexFilterDirectory) => {
