...
import type { CustomPlaylist, PlexTrack, QueueAlbumInput } from '../types';

export interface TrackListProps {
  tracks?: PlexTrack[] | null;
  albumData?: QueueAlbumInput | null;
...
function TrackList({ tracks, albumData, onPlayTrack, currentTrack, isPlaying, onTogglePlayback, onTrackAddedToPlaylist }: TrackListProps) {
  const [queuedTracks, setQueuedTracks] = useState<Set<string>>(new Set());
  const [, forceUpdate] = useState({});
  const [addToPlaylistTrack, setAddToPlaylistTrack] = useState<PlexTrack | null>(null);
  ...
  const updateQueuedTracks = async () => {
    try {
      ...
      const queuedTrackKeys = new Set<string>(queueArray.map(item => item.track.ratingKey));
      setQueuedTracks(queuedTrackKeys);
    } catch (error) {
      console.error('Error updating queued tracks:', error);
      setQueuedTracks(new Set<string>());
    }
  };
  ...
  const handleTrackClick = (track: PlexTrack) => {
  ...
  const handleDownload = async (event: React.MouseEvent<HTMLButtonElement>, track: PlexTrack) => {
  ...
  const handleAddToQueue = async (event: React.MouseEvent<HTMLButtonElement>, track: PlexTrack) => {
    event.stopPropagation();
    try {
  ...
  const handleOpenAddToPlaylist = (event: React.MouseEvent<HTMLButtonElement>, track: PlexTrack) => {
    event.stopPropagation();
  ...
  const handlePlaylistCreated = (playlist: CustomPlaylist) => {
  ...
  const isTrackInQueue = (track: PlexTrack) => queuedTracks.has(track.ratingKey);
