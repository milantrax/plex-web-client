...
import type { CustomPlaylist, PlexTrack } from '../types';
...
export interface AddToPlaylistModalProps {
  ...
  onClose: () => void;
  onOpenCreate: () => void;
  onTrackAdded?: (playlistId: number) => void;
}
...
export default function AddToPlaylistModal({ open, track, onClose, onOpenCreate, onTrackAdded }: AddToPlaylistModalProps) {
  ...
  const handleAdd = async (playlist: CustomPlaylist) => {
    if (!track) return;
    ...
    try {
      ...
      const msg = getApiErrorMessage(err, 'Failed to add track');
      setFeedback({ severity: getApiErrorStatus(err) === 409 ? 'info' : 'error', message: msg });
    } finally {
      setAdding(null);
    }
  };
