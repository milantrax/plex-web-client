...
import type { CustomPlaylistPlexTrack, SnackbarSeverity, SnackbarState } from '../types';
...
const PlaylistPage = () => {
  ...
  const [tracks, setTracks] = useState<CustomPlaylistPlexTrack[]>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        if (type === 'plex') {
          ...
          setTracks(tracksArr as CustomPlaylistPlexTrack[]);
  ...
  const handleTracksChange = (newTracks: CustomPlaylistPlexTrack[]) => setTracks(newTracks);
  const handleRemoveTrack = (track: CustomPlaylistPlexTrack) => setTracks(prev => prev.filter(t => t._customTrackId !== track._customTrackId));
