...
import type { AlbumCardWidthChangedEvent, PlexAlbum, PlexTrack } from '../types';

/** Playback props supplied by the SearchWrapper in App. */
export interface SearchProps {
  currentTrack?: PlexTrack | null;
  isPlaying?: boolean;
  onPlayTrack?: (track: PlexTrack) => void;
  onTogglePlayback?: () => void;
  onCurrentTrackChange?: Dispatch<SetStateAction<PlexTrack | null>>;
}

interface SearchInnerProps extends SearchProps {
  /** Minimal stand-in for the router location, built by the wrapper. */
...
class Search extends React.Component<SearchInnerProps, SearchState> {
  constructor(props: SearchInnerProps) {
    super(props);
    this.state = {
      albums: [],
      isLoading: false,
  ...
  handleWidthChange = (event: AlbumCardWidthChangedEvent) => {
    this.setState({ cardWidth: event.detail.width });
  };

  componentDidUpdate(prevProps: SearchInnerProps) {
  ...
  render() {
    ...
    return (
      <Box sx={{ px: 2.5, pt: `${NAVBAR_HEIGHT + 20}px`, pb: `${PLAYER_HEIGHT + 20}px` }} style={{ '--card-width': `${cardWidth}px` } as React.CSSProperties}>
        {query && (
          <Box sx={{ mb: 2.5 }}>
...
const SearchWithRouter = (props: SearchProps) => {
